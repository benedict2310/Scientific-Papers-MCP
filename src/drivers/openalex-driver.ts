...
export class OpenAlexDriver extends BaseDriver {
   ...
   */
  async fetchTopCited(concept: string, since: string, count: number): Promise<PaperMetadata[]> {
    if (!this.checkRateLimit()) {
      const retryAfter = this.getRetryAfter();
   ...
   */
  private async convertWorkToPaper(work: OpenAlexWork, includeText: boolean = false): Promise<PaperMetadata> {
    // Extract authors
